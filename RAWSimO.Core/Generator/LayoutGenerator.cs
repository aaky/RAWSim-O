    ...
    class LayoutGenerator
    {
        ...
        public void fillTiers()
        {
            ...
            foreach (var tier in instance.Compound.Tiers)
            {
                ...
                ShiftStorageDestinationCoordinates();
                instance.Flush();               
                locateResourceFiles();
            }
        ...
        public void ShiftStorageDestinationCoordinates()
        {
            int podCount = (int)Math.Floor(lc.PodAmount * (lc.nStorageBlocks() * lc.nStorageLocationsPerBlock()));
            double INNER_AISLE_POD_ORIENTATION = 2 * Math.PI - Math.PI / 2;
            double OUTER_AISLE_POD_ORIENTATION = Math.PI / 2;

            //Modify Pod waypoints by one grid outwards
            for (int i = 0; i<podCount; i++)
            {
                double temp_aislewaypoint_y = instance.Pods[i].Waypoint.Y;
                double temp_aislewaypoint_x = instance.Pods[i].Waypoint.X;
                temp_aislewaypoint_y += 1.0;
                instance.Pods[i].Waypoint.PodPickingOrientation = INNER_AISLE_POD_ORIENTATION;

                for (int j = 0; j<podCount; j++)
                {
                    if(instance.Pods[j].Waypoint.Y == temp_aislewaypoint_y)
                    {
                        temp_aislewaypoint_y -= 2.0;
                        instance.Pods[i].Waypoint.PodPickingOrientation = OUTER_AISLE_POD_ORIENTATION;
                        break;
                    }
                }
                
                instance.Pods[i].Waypoint.AisleWaypointY = temp_aislewaypoint_y;

                for(int k = 0; k < instance.Waypoints.Count; k++)
                {
                    if((instance.Waypoints[k].X == temp_aislewaypoint_x) && (instance.Waypoints[k].Y == temp_aislewaypoint_y))
                    {
                        instance.Pods[i].Waypoint.AisleID = instance.Waypoints[k].ID;
                    }
                }
            }
        }
